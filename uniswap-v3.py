...
llama_url = "https://api.llama.fi/protocol/uniswap-v3"
response = requests.get(llama_url)
data = response.json()

# chain_list = ['Optimism', 'Polygon', 'Celo', 'Arbitrum', 'Ethereum', 'Binance']
chain_list = ['Optimism', 'Polygon', 'Celo', 'Arbitrum', 'Ethereum']
...
plt.gca().yaxis.set_major_formatter(mtick.FuncFormatter(lambda x, _: '${:,.0f}M'.format(x/1000000)))
